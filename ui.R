      ...
      tabsetPanel(id="main",
          tabPanel("About",
             ...
             column(8,
               ...
               h3("Introduction"),
               h5("This application is designed to assist researchers investigating the effects of global change on ecosystem multifunctionality (i.e. GC-EMF research) to develop mechanistic hypotheses. The application accompanies Giling et al. (in prep). Please see the paper for a comprehensive description of the framework presented here."),
               h5("The user provides hypothesized effect sizes for 1) the effect of a global change driver on biotic and/or abiotic mechanisms that drive functioning (e.g. species diversity or water availability), and 2) the influence of these mechanisms on multiple individual functions. The application then generates a simple dataset based on these hypotheses to explore the multifunctionality responses using both the average and threshold approaches. In particular, these simulations can be used to quickly visualize and disentangle how the interacting ecological and mathematical effects underlying the conceptual model influence the emergent GC-EMF relationship."),
               br(),
               
               h3("Instructions"),
               h5("Hypotheses are entered in the sidebar on the left. Each equation represents one or two arrows from the conceptual model below. A zero (e.g. '0*A1' or just '0') indicates no effect. Interactive effects can also be added (e.g. '1*A1*B1'). The arrow ‘a’ can be set as pointing left or right (but not bidirectional). To exclude arrow a, simply do not include M1 or M2 as a predictor variable in Eqn. 1 or 2."),
